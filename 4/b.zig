...
const BingoSubsystem = struct {
    ...
    fn hasRemainingNumbers(self: BingoSubsystem) bool {
        return self.index < self.numbers.len;
    }
    ...
    fn next(self: *BingoSubsystem) ?u64 {
        ...
        while (i < self.boards.len) {
            if (self.boards[i].mark(number)) {
                // board is a winning board
                result = self.boards[i].sum() * number;
                // remove it from the lineup
                self.boards[i] = self.boards[self.boards.len - 1];
                self.boards = self.boards[0..self.boards.len - 1];
            } else {
                i += 1;
            }
        }
...
pub fn main() !void {
    ...
    var last_winner: u64 = undefined;

    while (bingo.hasRemainingNumbers()) {
        if (bingo.next()) |sum| {
            last_winner = sum;
        }
    }
